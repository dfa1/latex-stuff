...
\begin{description}
    \item [a] calcolare la probabilità di assorbimento dello stato $0$ per la catena con matrice di transizione:
...
$$
    ...
    \item [b] calcolare la probabilità che la catena sia assorbita nello stato 0 se la densità iniziale è quella uniforme
...
Al fine di determinare la probabilità di assorbimento dello stato $0$ si usa la seguente formula:
...
$$x_1 - \frac{1}{9}x_1 = \frac{1}{3}$$. Infine, applicando la definizione di $\lambda$ si conclude che la probabilità di essere assorbiti nello stato $0$ è:
...
dove $p$ rappresenta la probabilità della uniforme. 
...
Poiché $\lambda_3^{\{0\}}$ è $0$ per definizione dato che lo stato 3 è assorbente. Si ottiene quindi:
...
sostiutendo $\frac{15}{8}$ in $x_1$ si ottiene:
